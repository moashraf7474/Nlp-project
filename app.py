...
# cached model/tokenizer/classes to avoid re-loading on every call
_MODEL = None
_TOKENIZER = None
_CLASSES = None
# -------------------------------------------------------------------
...
def _ensure_model_loaded():
    """Load model, tokenizer and class labels once and cache them.
    Tries a normal load first; if that fails (meta tensor issues), falls back to CPU load."""
    global _MODEL, _TOKENIZER, _CLASSES
    if _MODEL is not None and _TOKENIZER is not None and _CLASSES is not None:
        return

    # load tokenizer
    _TOKENIZER = DistilBertTokenizerFast.from_pretrained("expense_classifier_tokenizer")

    # try normal load first
    try:
        _MODEL = DistilBertForSequenceClassification.from_pretrained("expense_classifier_model")
    except Exception:
        # fallback: load on CPU to avoid meta tensor -> device copy errors
...
def classify_sentence(sentence):
    try:
        ...
        # ensure model/tokenizer/classes are loaded
        ...
        # Prepare device and try to use GPU if available
        preferred_device = "cuda" if torch.cuda.is_available() else "cpu"
        model_device = next(_MODEL.parameters()).device

        # Try moving model to preferred device (may fail if model uses meta tensors)
        try:
            if str(model_device) != preferred_device:
                _MODEL.to(preferred_device)
        ...
        inputs = _TOKENIZER(sent, return_tensors="pt", truncation=True, padding=True)
